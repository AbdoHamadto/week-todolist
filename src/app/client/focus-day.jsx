  ...
  return(
    <div className="w-11/12 flex justify-center items-center">
      <div className="w-2/6 flex flex-col items-center ">
        ...
        <div className="w-3/5 bg-bgCard p-2 rounded-lg flex flex-col items-center">
      ...
      <div className="w-4/6 bg-bgCard flex flex-col items-center h-[800px] rounded-lg">
        ...
        <ul className="py-3 px-4 bg-bgTaske rounded-b-3xl h-[550px] overflow-y-auto w-4/5">
          {data[id].tasks.map((item, index) => {return(
            <li key={index} className="relative group overflow-hidden text-lg bg-bgTaskes mt-2 mb-3 py-2 flex justify-between items-center">
              <p onClick={() => stateTask(id, index)} className={`${item.state && "text-red-900 line-through"} w-full cursor-pointer mx-3 text-darkblue text-xl`}>{item.task}</p>
              <Image 
                src={deleteTask}
                alt='removeTask'
        ...
        <button 
          ...
          className="w-3/5 p-2 bg-bgTaske hover:bg-bgTaskes hover:text-bgRemove my-8 font-bold text-xl"
        >
          حذف الكل
        </button>
        <Link href="/" className="w-2/5 p-2 bg-bgTaske hover:bg-bgTaskes hover:text-bgRemove font-bold text-center">رجوع</Link>
      </div>
    </div>
  )
}
